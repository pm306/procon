...
int main(){
	...
	//step1:グラフに必要な頂点集合を得る
	vector<ll> vertex; //1階,N階及びエレベーターが設置されている階の集合
	vertex.push_back(1);
	vertex.push_back(N);
	rep(i, M){
		vertex.push_back(A[i]);
		vertex.push_back(B[i]);
	}
	sort(all(vertex));
	UNIQUE(vertex); //重複削除
	int n = vertex.size(); //グラフの頂点数

	//step2:重み付きの辺を張る
	vector<vector<edge>> G(n);
	//徒歩
	rep(i, n-1){
		G[i].emplace_back(i+1, vertex[i+1] - vertex[i]);
		G[i+1].emplace_back(i, vertex[i+1] - vertex[i]);
	}
	//エレベーター
	rep(i, M){
		int u = lower_bound(all(vertex), A[i]) - vertex.begin();
		int v = lower_bound(all(vertex), B[i]) - vertex.begin();
		G[u].emplace_back(v, C[i]);
		G[v].emplace_back(u, C[i]);
	}

	//step3:ダイクストラで最短距離を求める
	min_priority_queue<pair<ll, int>> que; que.emplace(0, 0); //cost, v
	vector<ll> dist(n, LLINF); dist[0] = 0;
	while(que.size()){
		auto [cost, v] = que.top(); que.pop();
		if(dist[v] < cost)continue;
		for(auto [nv, ncost] : G[v]){
			if(chmin(dist[nv], dist[v]+ncost)){
