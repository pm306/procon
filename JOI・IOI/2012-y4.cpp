...
// 数え上げ。
// 制約条件を満たさない時、continueではなく0に更新してしまった。数え上げなので更新しない。
// 仮想的なメニューは最後に置くとループが簡単になる。
...
//ラスト2日のメニューを記録して数え上げる
mint dp[110][4][4];
int main(){
    int N, K; cin >> N >> K;
    vector<int> sc(N, -1); //パスタのスケジュール。3は何も食べていない＝制約がないことを表す
    rep(i, K){
        int d, p; cin >> d >> p; d--;
        sc[d] = p - 1;
    }
    //init
    dp[0][3][3] = 1;
    rep(i, N)rep(j, 4)rep(k, 4){ //j:=1日前、k:=2日前
        if(sc[i] == -1){
            rep(l, 3){
            ...
            if(j == k and k == sc[i])continue;
            dp[i+1][sc[i]][j] += dp[i][j][k];
        }
    }

    mint res = 0;
    rep(i, 3)rep(j, 3) res += dp[N][i][j];
    cout << res << ln;
}
