...
int main() {
    ...
    auto dfs2 = [&](auto dfs2, int v, int d_par, int p) -> int{
        vector<pii> d_child;
        d_child.emplace_back(0, -1); //番兵

        for(auto &e:g[v]){
            if(e.to == p)d_child.emplace_back(d_par + e.cost, e.to);
            else d_child.emplace_back(e.cost + dist[e.to], e.to);
        }

        sort(all(d_child), greater<pii>());
        int ret = d_child[0].first + d_child[1].first;

        for(auto &e:g[v]){
            if(e.to == p)continue;
            chmax(ret, dfs2(dfs2, e.to, d_child[d_child[0].second == e.to].first, v));
        }
        return ret;
    };
