...
int main(){
    ...
    vector<int> pazzle(9);
    rep(i, 8)pazzle[P[i]-1] = i+1;
    debug(pazzle);

    vector<int> anspaz = {1,2,3,4,5,6,7,8,0};
    if(anspaz == pazzle)drop(0);

    vector<int> PP = {0,1,2,3,4,5,6,7,8};
    map<vector<int>, int> memo; //配列→番号
    map<int, vector<int>> rmp; //番号→配列
    int idx = 1;
    ...
    queue<int> que;
    que.push(memo[pazzle]);
    vector<int> cost(1e6, INF);
    cost[memo[pazzle]] = 0;
    while(que.size()){
        int idx = que.front(); que.pop();
        auto vec = rmp[idx];
        int u = find(all(vec), 0) - vec.begin(); //空きマス
        for(auto v : G[u]){ //スワップ
            swap(vec[u], vec[v]);
            int idx_to = memo[vec2]; //交換後のid
            if(cost[idx_to] == INF){
                cost[idx_to] = cost[idx] + 1;
                que.push(idx_to);
            }
            swap(vec[u], vec[v]);
        }
    ...
    int ans_idx = memo[anspaz];
    if(cost[ans_idx] == INF)cout << -1 << ln;
    else cout << cost[ans_idx] << ln;
}
