...
int janken(char a, char b){
    if(a == b)return -1;
    if(a=='G' and b=='C' or a=='C' and b=='P' or a=='P' and b=='G')return 0;
    return 1;
}
...
int main() {
    ...
    vector<pii> rank(n); //勝利数, インデックス(勝利数を負にして降順ソート)
    rep(i, n)rank[i].second = i;
    rep(j, M){
        rep(i, N){
            int l = rank[2*i].second, r = rank[2*i+1].second;
            int res = janken(A[l][j], A[r][j]);
            if(res != -1) rank[2*i+res].first--;
        }
        sort(all(rank));
    }
    for(auto [win, idx]:rank)cout << idx+1 << ln;
}
