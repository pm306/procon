...
using edge = pii; //to, idx

//頂点sから頂点tまでの最短パスを辿る
void bfs(vector<vector<edge>>& g, int s, int t, vector<int>& cnt){
    int n = g.size();
    ...
    vector<int> path; path.push_back(t);
    int cur_dist = dist[t], cv = t;
    while(cur_dist){
        for(auto& [nv, idx] : g[cv]){
            if(dist[nv] == cur_dist - 1){
                cur_dist--;
                cv = nv;
                path.push_back(cv);
                break;
    ...
    while(path.size() > 1){
        int now = path.back(); path.pop_back();
        int next = path.back();
        for(auto& [nv, idx] : g[now]){
            if(nv == next){
                cnt[idx]++;
                break;
            }
...
int main() {
    ...
    //dp[i][j]:=辺をi個見て通った回数の合計がj(<=1e5)である組み合わせ
    vector dp(N, vector<mint>(1e5+1));
    dp[0][0] = 1;
    rep(i, N-1){
        rep(j, 1e5+1){
            dp[i+1][j] += dp[i][j];
    ...
    rep(red, 1e5+1){
        int blue = sum - red;
        if(abs(red - blue) == K)res = dp[N-1][red];
    }
    cout << res.val() << ln;
}
