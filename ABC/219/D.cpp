...
int main(){
    ...
    //dp[i][j][k]:=i個目まで見た時にたこやきがj個、たいやきがk個である場合の最小購入個数
    vector dp(N+1, vector(MAX, vector<int>(MAX, inf)));
    dp[0][0][0] = 0;
    rep(i, N)rep(j, MAX)rep(k, MAX){
        if(dp[i][j][k] == inf)continue;
        chmin(dp[i+1][j][k], dp[i][j][k]);
        chmin(dp[i+1][min(j+A[i], MAX-1)][min(k+B[i], MAX-1)], dp[i][j][k]+1);
    }

    int res = inf;
    rept(j, X, MAX)rept(k, Y, MAX)chmin(res, dp[N][j][k]);
