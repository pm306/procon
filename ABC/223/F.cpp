...
int main(){
    ...
    while(Q--){
        ...
        if(t==1){
            r--;
            if(memo[l]==-1 and memo[r]==1){
                seg.apply(l, N, 2);
                seg.apply(r, N, -2);
            }
            else if(memo[l]==1 and memo[r]==-1){
                seg.apply(l, N, -2);
                seg.apply(r, N, 2);
            }
            swap(memo[l], memo[r]);
            ...
            int last = seg.prod(r-1, r) - (l ? seg.prod(l-1, l) : 0);
            int mi = seg.prod(l, r) - (l ? seg.prod(l-1, l) : 0);
            if(last == 0 and mi >= 0)ans.push_back("Yes");
