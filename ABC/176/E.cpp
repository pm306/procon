...
int main() {
    ...
    //行、列ごとに最大値を更新しながら個数をカウントする
    for(auto [h, w] : st){
        chmax(hmaxval, ++mph[h]);
        chmax(wmaxval, ++mpw[w]);
    }

    //最大個数爆破できる行、列のインデックスを取得
    for(auto [idx, val] : mph){
        if(hmaxval == val) hmax.push_back(idx);
    }
    for(auto [idx, val] : mpw){
        if(wmaxval == val) wmax.push_back(idx);
    }

    //答えを仮置き。重複があるので-1
    int res = hmaxval + wmaxval - 1;
    for(auto h : hmax){
        for(auto w : wmax){
            //最大個数爆破できて、爆弾が置かれていないマスが１つでもあれば+1
