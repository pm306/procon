...
vector<int> topo_sort(vector<vector<int>> &g){
    ...
    auto dfs = [&](auto&& dfs, int v) -> bool{
        ...
        for(auto& nv:g[v]){
            ...
            if(seen[nv])continue;
            if(dfs(dfs, nv) == false)return false;
        }
        state[v] = false;
        ret.emplace_back(v);
        return true;
...
int main(){
    ...
    //試合ごとにインデックスを割り振る
    vector indexes(N, vector<int>(N));
    int idx = 0;
    rep(i, N)rept(j, i+1, N){
        indexes[i][j] = indexes[j][i] = idx++;
    }
    ...
    rep(i, N)rep(j, N-2){
        int u = indexes[i][A[i][j]];
        int v = indexes[i][A[i][j+1]];
        G[u].push_back(v);
    }

    //閉路があると試合が組めない(デッドロックみたいな感じ)ので-1を出力して終了
    ...
    vector<int> dp(n);
    for(auto& v:topo){
        for(auto& to:G[v])chmax(dp[to], dp[v]+1);
    }
    cout << *max_element(dp.begin(), dp.end()) + 1 << ln;
}
