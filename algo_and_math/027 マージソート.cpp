...
template<class T> vector<T> mergeSort(vector<T> vec, bool isAsc=true){
    int len = vec.size();
    if(len <= 1)return vec;
    ...
    //ソート
    vector<T> sv1 = mergeSort(v1, isAsc);
    vector<T> sv2 = mergeSort(v2, isAsc);
    ...
    for(int i=0; i<len; i++){
        //sv1が空
        if(idx1 == sv1.size()){
            res.push_back(sv2[idx2++]);
        }
        //sv2が空
        else if(idx2 == sv2.size()){
            res.push_back(sv1[idx1++]);
        }
        //それ以外
        else{
            //降順
            if(isAsc){
                if(sv1[idx1] <= sv2[idx2]){
                    res.push_back(sv1[idx1++]);
                }
                else{
                    res.push_back(sv2[idx2++]);
