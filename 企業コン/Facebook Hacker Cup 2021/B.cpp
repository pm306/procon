...
pii solve(){
    int n; cin >> n;
    vector<string> g(n); rep(i, n)cin >> g[i];

    vector<int> Counter(100);
    //1置きだけ重複するので、置いたマスを記録
    vector check(n, vector<int>(n, false));
    ...
    rep(j, n){
        ...
        if(cnt != -1){
            if(cnt == 1 and check[last][j])continue;
            Counter[cnt]++;
        }
    }
    pii res = {-1, -1};
    rep(i, 100)if(Counter[i]){
        res = {i, Counter[i]};
        break;
    }
    return res;
...
int main(){
    ...
    rep(i, T){
        if(ans[i].first == -1){
            cout << "Case #" << i+1 << ": " << "Impossible" << ln;
        }
        else{
            cout << "Case #" << i+1 << ": " << ans[i].first << " " << ans[i].second << ln;
        }
