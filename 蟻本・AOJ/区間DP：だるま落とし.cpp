...
int main(){
    while(true){
        ...
        auto dp = [&](auto dp, int l, int r) -> int{
            if(r - l <= 1) return 0; //区間1以下
            if(r - l == 2) return remove(l, r-1); //隣接区間

            if(memo[l][r] != -1) return memo[l][r]; //メモ化済み

            int ret = 0;
            //最後に左右を連結する場合
            if(dp(dp, l+1, r-1) == r-l-2 and remove(l, r-1)) ret = r-l;
            //それ以外
            for(int k=l+1; k<r; k++){
                chmax(ret, dp(dp, l, k) + dp(dp, k, r));
            }
            return memo[l][r] = ret;
        };
        cout << dp(dp, 0, N) << ln;
