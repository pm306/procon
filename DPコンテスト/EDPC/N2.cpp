...
int main(){
    ...
    //dp[l][r]:=区間[l,r)までのスライムを合体させた時の最適値
    vector memo(500, vector<ll>(500, LLINF));
    auto dp = [&](auto dp, int l, int r) -> ll{
        if(r - l == 1) return 0;

        if(memo[l][r] != LLINF) return memo[l][r];

        ll ret = LLINF;
        for(int k=l+1; k<r; k++){
            chmin(ret, dp(dp, l, k) + dp(dp, k, r) + (S[k]-S[l]) + (S[r]-S[k]));
        }
        return memo[l][r] = ret;
    };
    cout << dp(dp, 0, N) << ln;
